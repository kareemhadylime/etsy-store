...
- [x] **Repo first-impressions trio ✅** (2026-05-11) → real `README.md` replacing the create-next-app boilerplate (CI badge, what-this-is summary, stack, repo layout, run/test/lint/build commands, CI section, deploy-via-runbook pointer, phase status table) + `.github/dependabot.yml` (weekly npm + Actions bumps, grouped minor/patch, Next/React majors ignored per AGENTS.md) + `.github/pull_request_template.md` (what/why/test plan/docs-touched checklist mirroring runbook DNA). Runbook section 11 extended with Dependency Maintenance + PR Template subsections.
- [x] **Migration replay CI job ✅** (2026-05-11) → second parallel CI job (`migrations`) runs alongside `test`. Spins up `postgres:16-alpine` service container, applies `supabase/test-shim.sql` (minimal `auth` schema + `auth.users` stub table + stub functions for `auth.role()` / `auth.uid()` / `auth.jwt()`), then replays all 14 files in `supabase/migrations/` in order with `psql -v ON_ERROR_STOP=1`. Catches malformed SQL, ordering bugs, missing extensions, broken `IF NOT EXISTS` idempotency — the "broken migration ships to main" failure mode that nothing else in CI catches. NOT a faithful RLS replay (auth.role() always returns service_role in the shim); for real RLS testing, use a Supabase preview branch per runbook section 3. Runbook section 11 extended with a "Migration replay job" subsection covering what's caught/not-caught.
- [x] **Baseline security headers ✅** (2026-05-11) → `src/lib/security/headers.ts` exports `getSecurityHeaders()`; `next.config.ts` `headers()` function applies them to `'/(.*)'` (every response). Five headers: HSTS 2-year/includeSubDomains/preload, X-Content-Type-Options nosniff, X-Frame-Options DENY, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy disabling camera/mic/geolocation/payment/etc. **CSP intentionally deferred** — needs careful allowlisting of analytics/Klaviyo/GA4/Meta pixel origins, easy to silently break tracking. 9 new tests (447 total). Runbook section 13 documents headers + the CSP-deferral rationale + verifying-after-deploy steps.
